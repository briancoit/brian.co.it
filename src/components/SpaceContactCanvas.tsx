...
export function SpaceContactCanvas(): React.JSX.Element {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      targetParallaxRef.current = Math.max(
        0,
        Math.min(1.2, (vh - rect.top) / (vh + rect.height)),
      );
    };
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < starCount; i++) {
      ...
      sizes[i] =
        seed < 0.8 ? 0.8 + Math.random() * 1.5 : 2.5 + Math.random() * 2.0;
    ...
    const starMat = new ShaderMaterial({
      ...
      vertexShader: `
				...
				void main() {
					...
					float twinkleBase = 0.75 + 0.25 * sin(time * frequency + phase); // Subtle pulse
					float twinkle = mix(1.0, twinkleBase, isTwink);
					
					vAlpha = extraData.z * (0.4 + 1.0 * twinkle); // Stable alpha
					gl_PointSize = gl_PointSize * (0.9 + 0.2 * twinkle); // Subtle size pulse
					gl_PointSize = max(gl_PointSize, 2.0 * pixelRatio);
					vGlow = extraData.x;
					
                    // CHROMATIC SCINTILLATION
                    float scin = extraData.w;
                    float flicker = sin(time * 6.0 + phase * 8.0); // Slightly faster shift
    ...
    const posBuffer = new Float32BufferAttribute(
      new Float32Array(maxConnections * 3 * 2),
      3,
    );
    const colorBuffer = new Float32BufferAttribute(
      new Float32Array(maxConnections * 3 * 2),
      3,
    );
    ...
    function writeLine(
      ...
      const i3 = index * 6;
      posArr[i3] = n1.pos.x;
      posArr[i3 + 1] = n1.pos.y;
      posArr[i3 + 2] = n1.pos.z;
      posArr[i3 + 3] = n2.pos.x;
      posArr[i3 + 4] = n2.pos.y;
      posArr[i3 + 5] = n2.pos.z;
      ...
      colArr[i3 + 3] = r * intensity;
      colArr[i3 + 4] = g * intensity;
      colArr[i3 + 5] = b * intensity;
    }
    ...
    function animate(now: number) {
      ...
      if (networkRef.current) {
        const { nodes, edges, edgeAttribute, colorAttribute } =
          networkRef.current;
        ...
        const friction = 0.5 ** deltaTime; // Exponential decay friction
